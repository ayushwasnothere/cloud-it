...
const XTermComponent = dynamic(
  ...
  {
    loading: () => (
      <div className="flex h-full items-center justify-center bg-[#0b0b0d]">
...
export function Terminal({ projectId, isRunning, onSessionExit }: TerminalProps) {
  ...
  const statusColor = useMemo(() => {
    if (!isRunning) return 'bg-neutral-600'
  ...
  return (
    <div className="flex h-full flex-col border-t border-[#2a2a2f] bg-[#0b0b0d]">
      <div className="flex items-center justify-between border-b border-[#2a2a2f] bg-[#151519] px-3 py-2 text-xs">
        <div className="flex items-center gap-2 text-neutral-300">
          <div className={`h-2 w-2 rounded-full ${statusColor}`} />
          <span>{statusLabel}</span>
          {isConnected && (
            <span className="rounded border border-[#3b3d45] bg-[#1a1b20] px-1.5 py-0.5 text-[10px] text-neutral-300">
              Press Enter
            </span>
          )}
        </div>
        ...
        {error && (
          <div className="flex max-w-[70%] items-center gap-1 text-red-400">
        ...
        {closeReason && !isConnected && !error && (
          <span className="max-w-[70%] truncate text-neutral-500">{closeReason}</span>
        )}
      </div>
